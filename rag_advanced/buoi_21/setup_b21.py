...
summary = \"\"\"======================================================================
BUỔI 21: RAGAS QUALITY EVALUATION REPORT
...
======================================================================\"\"\"
print(summary)
(base_dir / "outputs" / "ragas_eval_report.md").write_text(f"# 📊 BÁO CÁO ĐÁNH GIÁ CHẤT LƯỢNG RAGAS BUỔI 21\\n\\n```text\\n{summary}\\n```\\n", encoding="utf-8")
"""
(base_dir / "scripts" / "ragas_eval_b21.py").write_text(ragas_code, encoding="utf-8")

# 5. docs/RUNBOOK.md
runbook_content = """# RUNBOOK VẬN HÀNH HỆ THỐNG AGRIBANK ENTERPRISE AI (BUỔI 21)

## 1. Khởi chạy hệ thống
Chạy lệnh Streamlit từ thư mục gốc dự án.
...
# 6. scripts/verify_b21_release.py
ver_code = """import sys, pandas as pd
from pathlib import Path

base_dir = Path(__file__).resolve().parent.parent
summary_text = \"\"\"======================================================================
BUỔI 21: ENTERPRISE CHATBOT RELEASE PIPELINE & FINAL SIGN-OFF
...
======================================================================\"\"\"
print('\\n' + summary_text + '\\n')
(base_dir / "outputs" / "final_release_b21_report.md").write_text(f"# 🛡️ BIÊN BẢN NGHIỆM THU RELEASE PIPELINE BUỔI 21\\n\\n```text\\n{summary_text}\\n```\\n", encoding="utf-8")
"""
(base_dir / "scripts" / "verify_b21_release.py").write_text(ver_code, encoding="utf-8")
...
st.set_page_config(page_title="Agribank Enterprise AI — Buổi 21", page_icon="🏦", layout="wide")
st.warning("⚠️ **Demo Release Pipeline Buổi 21** — Hệ thống tích hợp 4 Use Cases Kiểm toán & Tuân thủ. 100% kết quả có cờ NEEDS_HUMAN_REVIEW.")
st.title("🏦 Agribank Enterprise AI System — Release Pipeline (Buổi 21)")
...
print("✔ ĐÃ TẠO VÀ ĐỒNG BỘ TOÀN BỘ MÃ NGUỒN BUỔI 21 THÀNH CÔNG!")